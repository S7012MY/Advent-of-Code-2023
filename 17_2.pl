...
while (@q) {
  ...
  if ($i == @map - 1 && $j == @{$map[@map - 1]} - 1 && $consec > $MIN_CONS) {
    $bestHeat = $minHeat[$i][$j][$d][$consec] if $minHeat[$i][$j][$d][$consec] < $bestHeat;
  }
  
  my $newD;
  my ($ii, $jj) = ($i + $di[$d], $j + $dj[$d]);
  if (isInside($ii, $jj) && $consec < $MAX_CONS && (!$minHeat[$ii][$jj][$d][$consec + 1] || $minHeat[$ii][$jj][$d][$consec + 1] > $minHeat[$i][$j][$d][$consec] + $map[$ii][$jj])) {
    $minHeat[$ii][$jj][$d][$consec + 1] = $minHeat[$i][$j][$d][$consec] + $map[$ii][$jj];
    push @q, [$ii, $jj, $d, $consec + 1];
  }
  $newD = ($d + 1) % 4;
  ($ii, $jj) = ($i + $di[$newD], $j + $dj[$newD]);
  if (isInside($ii, $jj) && $MIN_CONS < $consec && (!$minHeat[$ii][$jj][$newD][0] || $minHeat[$ii][$jj][$newD][0] > $minHeat[$i][$j][$d][$consec] + $map[$ii][$jj])) {
